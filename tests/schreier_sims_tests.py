...
from cgt.schreier_sims import RandomSchreierSimsGenerator
...
class TestRandomSchreierSimsGenerator(unittest.TestCase):

    def test_trivial(self):
        cf = lambda x: Permutation.read_cycle_form(x, 4)
        a = cf([[1, 2]])
        b = cf([])
        c = cf([[1,2,3]])
        alg = RandomSchreierSimsGenerator([a], order=2)
        alg.complete()
        struct = alg.structure
        self.assertTrue(a in struct)
        self.assertTrue(b in struct)
        self.assertFalse(c in struct)
    ...
    def test_schreier_sims_algorithm(self):
        ...
        e = cf([], 4)
        f = cf([[1, 2, 3]], 4)
        alg = RandomSchreierSimsGenerator([a, b], order=24)
        alg.complete()
        struct = alg.structure
        self.assertTrue(a in struct)
        self.assertTrue(b in struct)
        ...
        self.assertTrue(e in struct)
        self.assertTrue(f in struct)
        alg = RandomSchreierSimsGenerator([c, d], order=12)
        alg.complete()
        struct = alg.structure
        self.assertFalse(a in struct)
        self.assertFalse(b in struct)
    ...
    def test_single_schreier_generator(self):
        ...
        a = cf([[1, 2, 5, 3, 4]])
        b = cf([[4, 5]])
        alg = RandomSchreierSimsGenerator([a, b], order=120)
        alg.complete()
        struct = alg.structure
        self.assertEqual(struct.order(), 120)
...
def all_tests_suite():
    ...
    suite.addTest(unittest.makeSuite(TestRandomSchreierSimsGenerator))
    return suite
