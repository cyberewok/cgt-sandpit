def traditional_backtrack_subgroup(super_group, group_property, cull_heuristic, size):
    pbw = PartitionBacktrackWorkhorse(group_property.check, cull_heuristic, size)
    return pbw.find_partition_backtrack_subgroup()

class TraditionalBacktrackWorkhorse():
    def __init__(self, prop_check, cull_heuristic, size):
        self.size = size
